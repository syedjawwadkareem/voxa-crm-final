...
//   const [showRoleSelectorDesktop, setShowRoleSelectorDesktop] = useState(false);
//   const [showRoleSelectorMobile, setShowRoleSelectorMobile] = useState(false);
//   const roleSelectorDesktopRef = useRef<HTMLDivElement>(null);
//   const roleSelectorMobileRef = useRef<HTMLDivElement>(null);
...
//       if (roleSelectorDesktopRef.current && !roleSelectorDesktopRef.current.contains(e.target as Node)) {
//         setShowRoleSelectorDesktop(false);
//       }
//       if (roleSelectorMobileRef.current && !roleSelectorMobileRef.current.contains(e.target as Node)) {
//         setShowRoleSelectorMobile(false);
//       }
//     };
...
//           <button onClick={() => setSidebarOpen(!sidebarOpen)} className="btn-outline lg:hidden">
//             ☰
//           </button>
//           <span className="logo-mark">VOXA</span>

//           {/* Desktop/laptop role selector - white, sits right next to the logo */}
//           <div className="relative hidden lg:block" ref={roleSelectorDesktopRef}>
//             <button
//               type="button"
//               aria-haspopup="listbox"
//               aria-expanded={showRoleSelectorDesktop}
//               className="flex items-center gap-2 bg-white hover:bg-slate-50 text-slate-700 text-sm font-medium pl-3 pr-2.5 py-1.5 rounded-full border border-slate-200 shadow-sm transition-colors"
//               onClick={() => setShowRoleSelectorDesktop(!showRoleSelectorDesktop)}
//             >
//               <span className="w-1.5 h-1.5 rounded-full bg-cyan-500 flex-shrink-0" />
//               <span className="max-w-[160px] truncate">{roleLabels[userRole]}</span>
//               <svg
//                 className={`w-3.5 h-3.5 text-slate-400 flex-shrink-0 transition-transform ${showRoleSelectorDesktop ? 'rotate-180' : ''}`}
//                 fill="none"
//                 stroke="currentColor"
//                 viewBox="0 0 24 24"
//               >
...
//               </svg>
//             </button>
//             {showRoleSelectorDesktop && (
//               <div
//                 role="listbox"
//                 className="absolute left-0 top-full mt-2 bg-white border border-slate-200 rounded-xl shadow-lg z-50 w-56 overflow-hidden"
//               >
...
//             {/* Compact role selector - mobile & tablet only */}
//             <div className="relative lg:hidden" ref={roleSelectorMobileRef}>
//               <button
//                 type="button"
//                 aria-haspopup="listbox"
//                 aria-expanded={showRoleSelectorMobile}
...
//                 onClick={() => setShowRoleSelectorMobile(!showRoleSelectorMobile)}
//               >
//                 <span className="w-1.5 h-1.5 rounded-full bg-cyan-500 flex-shrink-0" />
//                 <span className="hidden sm:inline max-w-[140px] truncate">{roleLabels[userRole]}</span>
//                 <svg
//                   className={`w-3.5 h-3.5 text-slate-400 flex-shrink-0 transition-transform ${showRoleSelectorMobile ? 'rotate-180' : ''}`}
//                   fill="none"
//                   stroke="currentColor"
//                   viewBox="0 0 24 24"
//                 >
...
// Platform Side: super_admin, billing_platform, did_manager, onboarding
// Tenant Side:   tenant_admin, agent, supervisor, billing_tenant
type UserRole =
  | 'super_admin'
  | 'billing_platform'
  | 'did_manager'
...
interface RolePermission {
  ...
  // Reports
  canViewReports: boolean;
  // IVR / queues / campaigns
  canManageIVR: boolean;
  canManageQueues: boolean;
  canManageCampaigns: boolean;
  canMonitorAgents: boolean;
  // Security / settings
  canViewSecurity: boolean;
  canModifySettings: boolean;
}
...
const rolePermissions: Record<UserRole, RolePermission> = {
  ...
  super_admin: {
    ...
    canViewBilling: true,
    canModifyBilling: true,
    canAccessSoftphone: false,   // platform admins don't use softphone
    canMakeOutbound: false,
    canAnswerInbound: false,
    canViewReports: true,
    canManageIVR: false,
  ...
  onboarding: {
    canAccessAdmin: false,
    canManageTenants: true,  // create / configure / activate tenants
    canManageDIDs: false,
    canManageUsers: false,
    canManageRoles: false,
    canViewBilling: false,
  ...
  tenant_admin: {
    canAccessAdmin: false,
    canManageTenants: false,
    canManageDIDs: true,          // own company DIDs
    canManageUsers: true,
    canManageRoles: false,
    canViewBilling: true,
    canModifyBilling: true,
  ...
  agent: {
    ...
    canMakeOutbound: true,
    canAnswerInbound: true,
    canViewReports: false,       // only own call history (shown in softphone)
    canManageIVR: false,
    canManageQueues: false,
    canManageCampaigns: false,
    canMonitorAgents: false,
  ...
  billing_tenant: {
    ...
    canManageRoles: false,
    canViewBilling: true,
    canModifyBilling: false,      // view-only for tenant billing user
    canAccessSoftphone: false,
    canMakeOutbound: false,
    canAnswerInbound: false,
    canViewReports: false,
...
// Group label shown in the switcher dropdown
const roleGroup: Record<UserRole, string> = {
  super_admin: 'Platform',
  billing_platform: 'Platform',
  did_manager: 'Platform',
  onboarding: 'Platform',
  tenant_admin: 'Tenant',
  agent: 'Tenant',
  supervisor: 'Tenant',
...
export default function VOXA() {
  ...
  const getPermittedPages = useCallback(() => {
    const pages = [
      { key: 'admin',    label: 'Admin Panel',       required: 'canAccessAdmin' as keyof RolePermission },
      { key: 'tenants',  label: 'Tenant Management', required: 'canManageTenants' as keyof RolePermission },
      { key: 'numbers',  label: 'DID Management',    required: 'canManageDIDs' as keyof RolePermission },
      { key: 'billing',  label: 'Global Billing',    required: 'canViewBilling' as keyof RolePermission },
      { key: 'softphone',label: 'Softphone',         required: 'canAccessSoftphone' as keyof RolePermission },
      { key: 'outbound', label: 'Outbound Calls',    required: 'canMakeOutbound' as keyof RolePermission },
      { key: 'inbound',  label: 'Inbound Calls',     required: 'canAnswerInbound' as keyof RolePermission },
      { key: 'queue',    label: 'Call Queue',        required: 'canAnswerInbound' as keyof RolePermission },
      { key: 'ivr',      label: 'IVR Menu',          required: 'canManageIVR' as keyof RolePermission },
      { key: 'omni',     label: 'OmniChannel',       required: 'canAccessSoftphone' as keyof RolePermission },
      { key: 'logs',     label: 'Master Logs',       required: 'canViewReports' as keyof RolePermission },
      { key: 'users',    label: 'Users',             required: 'canManageUsers' as keyof RolePermission },
      { key: 'security', label: 'Security',          required: 'canViewSecurity' as keyof RolePermission },
    ];
    return pages.filter((p) => hasPermission(p.required));
  }, [hasPermission]);
  ...
  const initials = (n: string) => {
    return n.split(/\s+|_/).map((x) => x[0] || '').join('').slice(0, 2).toUpperCase();
  };
  ...
  // DID Management Functions
  const createDid = () => {
    if (!didForm.phone || !didForm.extension) { showToast('Fill all required fields'); return; }
    const newDid = {
      id: `DID${String(dids.length + 1).padStart(3, '0')}`,
      phone: didForm.phone, type: didForm.type as 'Inbound' | 'Outbound',
      status: 'Active' as const, tenant: didForm.tenant, extension: didForm.extension,
      created: new Date().toLocaleDateString(), provider: didForm.provider,
    };
    setDids([...dids, newDid]);
    setCreateDidModal(false);
    setDidForm({ phone: '', type: 'Inbound', tenant: 'Phoenix Telecom Inc.', extension: '', provider: 'Twilio' });
  ...
  const updateDid = () => {
    if (!selectedDid || !didForm.phone || !didForm.extension) { showToast('Fill all required fields'); return; }
    setDids(dids.map(d => d.id === selectedDid.id ? { ...selectedDid, ...didForm, type: didForm.type as 'Inbound' | 'Outbound' } : d));
    setEditDidModal(false); setSelectedDid(null);
    setDidForm({ phone: '', type: 'Inbound', tenant: 'Phoenix Telecom Inc.', extension: '', provider: 'Twilio' });
    showToast(`DID ${didForm.phone} updated successfully`);
  };

  const deleteDid = (id: string) => { setDids(dids.filter(d => d.id !== id)); showToast('DID deleted successfully'); };
  ...
  // Tenant Management Functions
  const createTenant = () => {
    if (!tenantForm.name || !tenantForm.owner) { showToast('Fill all required fields'); return; }
    const newTenant = {
      id: `TENANT${String(tenants.length + 1).padStart(3, '0')}`,
      name: tenantForm.name, owner: tenantForm.owner, status: 'Active' as const,
      users: 0, dids: 0, created: new Date().toLocaleDateString(),
      tier: tenantForm.tier as 'Starter' | 'Professional' | 'Enterprise',
    };
    setTenants([...tenants, newTenant]);
    setCreateTenantModal(false); setTenantForm({ name: '', owner: '', tier: 'Professional' });
    showToast(`Tenant ${tenantForm.name} created successfully`);
  };

  const updateTenant = () => {
    if (!selectedTenant || !tenantForm.name || !tenantForm.owner) { showToast('Fill all required fields'); return; }
    setTenants(tenants.map(t => t.id === selectedTenant.id ? { ...selectedTenant, ...tenantForm, tier: tenantForm.tier as 'Starter' | 'Professional' | 'Enterprise' } : t));
    setEditTenantModal(false); setSelectedTenant(null); setTenantForm({ name: '', owner: '', tier: 'Professional' });
    showToast(`Tenant ${tenantForm.name} updated successfully`);
  };

  const deleteTenant = (id: string) => {
    if (dids.some(d => tenants.find(t => t.id === id)?.name === d.tenant)) { showToast('Cannot delete tenant with active DIDs'); return; }
    setTenants(tenants.filter(t => t.id !== id)); showToast('Tenant deleted successfully');
  };
  ...
  // User Management Functions
  const createUser = () => {
    if (!userForm.name || !userForm.role) { showToast('Fill all required fields'); return; }
    const newUser = {
      id: `USER${String(users.length + 1).padStart(3, '0')}`, name: userForm.name, role: userForm.role,
      status: 'Active' as const, owner: 'Aman_Smith', date: new Date().toLocaleDateString(),
      color: ['#64748b', '#f59e0b', '#8b5cf6', '#0ea5e9', '#10b981'][Math.floor(Math.random() * 5)],
    };
    setUsers([...users, newUser]); setCreateUserModal(false); setUserForm({ name: '', role: 'Agent', status: 'Active' });
    showToast(`User ${userForm.name} created successfully`);
  };

  const updateUser = () => {
    if (!selectedUserData || !userForm.name || !userForm.role) { showToast('Fill all required fields'); return; }
    setUsers(users.map(u => u.id === selectedUserData.id ? { ...selectedUserData, ...userForm } : u));
    setEditUserModal(false); setSelectedUserData(null); setUserForm({ name: '', role: 'Agent', status: 'Active' });
    showToast(`User ${userForm.name} updated successfully`);
  };

  const deleteUser = (id: string) => { setUsers(users.filter(u => u.id !== id)); showToast('User deleted successfully'); };
  ...
  useEffect(() => {
    if (activePage === 'admin' && adminChartRef.current) {
      if (adminChartInstanceRef.current) adminChartInstanceRef.current.destroy();
      const ctx = adminChartRef.current.getContext('2d');
      if (ctx) {
        adminChartInstanceRef.current = new Chart(ctx, {
          type: 'line',
          data: {
            labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
            datasets: [{ label: 'Call Volume', data: [450, 520, 480, 610, 580, 420, 350], borderColor: '#22c1a5', backgroundColor: 'rgba(34, 193, 165, 0.1)', tension: 0.4, fill: true }],
          },
          options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, max: 700 } } } as any,
        });
      }
    }
  }, [activePage]);
  ...
  useEffect(() => {
    if (activePage === 'omni' && omniChartRef.current) {
      if (omniChartInstanceRef.current) omniChartInstanceRef.current.destroy();
      const ctx = omniChartRef.current.getContext('2d');
      if (ctx) {
        omniChartInstanceRef.current = new Chart(ctx, {
          type: 'doughnut',
          data: { labels: ['Voice', 'WhatsApp', 'SMS', 'Email'], datasets: [{ data: [450, 320, 180, 150], backgroundColor: ['#22c1a5', '#3b82f6', '#f59e0b', '#8b5cf6'] }] },
          options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { position: 'bottom' } } } as any,
        });
      }
    }
  }, [activePage]);
  ...
  useEffect(() => {
    if (!callState.active) return;
    const interval = setInterval(() => { setSpTimer(fmtT((Date.now() - callState.start) / 1000)); }, 1000);
    return () => clearInterval(interval);
  }, [callState.active, callState.start]);
  ...
  useEffect(() => {
    if (callState.active && waveCanvasRef.current) {
      ...
      const waveTimer = setInterval(() => {
        ctx.clearRect(0, 0, c.width, c.height);
        ctx.strokeStyle = '#0f8f7a'; ctx.lineWidth = 1.5; ctx.beginPath();
        for (let i = 0; i < c.width; i++) {
          const y = c.height / 2 + Math.sin((i + x) * 0.1) * Math.random() * 20;
  ...
  // Active calls update
  useEffect(() => {
    const timer = setInterval(() => { setActiveKpiCalls((prev) => Math.max(0, prev + Math.floor(Math.random() * 3) - 1)); }, 4000);
    return () => clearInterval(timer);
  }, []);

  // Queue timer update
  useEffect(() => {
    const timer = setInterval(() => { setQueueData((prev) => prev.map((q) => ({ ...q, wait: q.wait + 1 }))); }, 1000);
    return () => clearInterval(timer);
  }, []);
  ...
  const SimulateIncoming = () => {
    ...
    const n = names[Math.floor(Math.random() * 4)];
    const num = `(555) ${Math.floor(100 + Math.random() * 899)}-${Math.floor(1000 + Math.random() * 8999)}`;
    setIncomingCallInfo({ name: n, num, avatar: initials(n) }); setIncomingModal(true);
  };

  useEffect(() => {
    const timer = setTimeout(() => { if (!callState.active) SimulateIncoming(); }, 20000);
    return () => clearTimeout(timer);
  }, [callState.active]);
  ...
  const AnswerIncoming = () => {
    ...
    if (inboundCalls.length > 0) {
      const c = inboundCalls[0];
      setSpNumber(c.num); setInboundCalls(inboundCalls.slice(1));
      switchPage('softphone'); setTimeout(() => toggleCall(), 100);
    }
  };

  const DeclineIncoming = () => {
    setIncomingModal(false);
    if (inboundCalls.length > 0) { setInboundCalls(inboundCalls.slice(1)); showToast('Declined'); }
  };
  ...
  const addIVRNode = (type: 'flow' | 'input') => {
    if (type === 'flow') {
      const newNode = { id: Date.now().toString(), icon: '📞', title: 'New Flow Step', detail: 'Configure this step', type: 'flow' as const };
      setIvrNodes({ ...ivrNodes, flow: [...ivrNodes.flow, newNode] }); showToast('Flow step added');
    }
  };

  const removeIVRNode = (id: string, type: 'trigger' | 'flow') => {
    if (type === 'trigger') setIvrNodes({ ...ivrNodes, trigger: ivrNodes.trigger.filter((n) => n.id !== id) });
    else setIvrNodes({ ...ivrNodes, flow: ivrNodes.flow.filter((n) => n.id !== id) });
    showToast('Node removed');
  };

  const addKeypadInput = () => {
    const nextKey = String((Math.max(...ivrNodes.inputs.map((i) => parseInt(i.key) || 0)) + 1) % 10);
    const newNode = { key: nextKey, node: { id: Date.now().toString(), icon: '📞', title: 'New Input', detail: '', type: 'input' as const } };
    setIvrNodes({ ...ivrNodes, inputs: [...ivrNodes.inputs, newNode] }); showToast('Keypad input added');
  };

  const updateKeypadNode = (index: number, key: string, title: string, icon: string) => {
    const updated = [...ivrNodes.inputs];
    updated[index] = { key, node: { ...updated[index].node, title, icon } };
    setIvrNodes({ ...ivrNodes, inputs: updated });
  };

  const removeKeypadInput = (index: number) => {
    setIvrNodes({ ...ivrNodes, inputs: ivrNodes.inputs.filter((_, i) => i !== index) }); showToast('Keypad input removed');
  };

  const bulkDial = () => {
    if (!bulkNumbers.trim()) { showToast('Enter phone numbers'); return; }
    const numbers = bulkNumbers.split('\n').filter((n) => n.trim());
    showToast(`Bulk dial queued: ${numbers.length} numbers`); setBulkDialModal(false); setBulkNumbers('');
  };
  ...
  // ─── Role switcher: grouped dropdown, always visible ────────────────────────
  const platformRoles: UserRole[] = ['super_admin', 'billing_platform', 'did_manager', 'onboarding'];
  const tenantRoles: UserRole[] = ['tenant_admin', 'agent', 'supervisor', 'billing_tenant'];
  ...
  const RoleSwitcher = () => (
    <div className="relative" ref={roleSelectorRef}>
      <button
        ...
        className="flex items-center gap-1.5 bg-white hover:bg-slate-50 text-slate-700 text-xs font-medium pl-2.5 pr-2 py-1.5 rounded-full border border-slate-200 shadow-sm transition-colors whitespace-nowrap"
        style={{ minWidth: 0 }}
      >
        {/* coloured dot — teal for platform, violet for tenant */}
        <span
          className="w-2 h-2 rounded-full flex-shrink-0"
          style={{ background: roleGroup[userRole] === 'Platform' ? '#22c1a5' : '#8b5cf6' }}
        />
        {/* Short side badge (always visible) */}
        <span className="text-[10px] font-semibold uppercase tracking-wide"
          style={{ color: roleGroup[userRole] === 'Platform' ? '#0d9488' : '#7c3aed' }}>
          {roleGroup[userRole]}
        </span>
        {/* Role label — hidden only on very small screens (below 360px) */}
        <span className="hidden min-[360px]:inline max-w-[120px] truncate text-slate-600">
          {roleLabels[userRole]}
        </span>
        <svg
      ...
      {showRoleSelector && (
        <div
          role="listbox"
          className="absolute right-0 top-full mt-2 bg-white border border-slate-200 rounded-xl shadow-lg z-50 w-60 max-w-[calc(100vw-1rem)] overflow-hidden"
        >
          {/* Platform group */}
          <div className="px-3 pt-2.5 pb-1">
            <div className="text-[10px] font-bold uppercase tracking-widest text-teal-600 flex items-center gap-1.5">
          ...
          <div className="pb-1">
            {platformRoles.map((role) => (
              <button
                ...
                aria-selected={userRole === role}
                className={`w-full text-left px-3 py-2 text-sm transition-colors ${
                  userRole === role ? 'bg-teal-50 text-teal-700 font-semibold' : 'text-slate-600 hover:bg-slate-50'
                }`}
                onClick={() => { setUserRole(role); setShowRoleSelector(false); showToast(`Role → ${roleLabels[role]}`); }}
              >
                {roleLabels[role]}
              </button>
            ))}
          ...
          {/* Tenant group */}
          <div className="px-3 pt-2.5 pb-1">
            <div className="text-[10px] font-bold uppercase tracking-widest text-violet-600 flex items-center gap-1.5">
          ...
          <div className="pb-1">
            {tenantRoles.map((role) => (
              <button
                ...
                aria-selected={userRole === role}
                className={`w-full text-left px-3 py-2 text-sm transition-colors ${
                  userRole === role ? 'bg-violet-50 text-violet-700 font-semibold' : 'text-slate-600 hover:bg-slate-50'
                }`}
                onClick={() => { setUserRole(role); setShowRoleSelector(false); showToast(`Role → ${roleLabels[role]}`); }}
              >
                {roleLabels[role]}
    ...
    <div className="flex min-h-screen">
      ...
      <aside className={`sidebar w-64 bg-[color:var(--sidebar)] text-slate-200 flex-shrink-0 flex flex-col ${sidebarOpen ? 'open' : ''}`}>
        <div className="px-5 py-5 flex items-center gap-2 border-b border-white/5">
          <svg className="w-8 h-8" viewBox="0 0 40 40" fill="none">
            <path d="M6 12 L10 28 L14 12 L18 28 L22 12 L26 28 L30 12 L34 28" stroke="url(#g1)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
            <defs><linearGradient id="g1" x1="0" x2="40"><stop stopColor="#22c1a5" /><stop offset="1" stopColor="#3b82f6" /></linearGradient></defs>
          </svg>
          <span className="logo-mark">VOXA</span>
      ...
      <main className="flex-1 min-w-0">
        {/* ── Topbar ─────────────────────────────────────────────────────── */}
        <div className="main-topbar flex items-center gap-2 flex-wrap">
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="btn-outline lg:hidden">☰</button>
          <span className="logo-mark">VOXA</span>

          {/* Role switcher — single instance, always visible at every breakpoint */}
          <RoleSwitcher />
        ...
        {activePage === 'admin' && (
          <section className="page active">
            ...
            <div className="px-8 mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="kpi"><div className="text-xs text-slate-500">Total Users</div><div className="text-2xl font-bold mt-1">142</div><div className="text-xs text-emerald-600 mt-1">▲ 12 this month</div></div>
              <div className="kpi"><div className="text-xs text-slate-500">Active Calls</div><div className="text-2xl font-bold mt-1">{activeKpiCalls}</div><div className="text-xs text-slate-500 mt-1">Live now</div></div>
              <div className="kpi"><div className="text-xs text-slate-500">Calls Today</div><div className="text-2xl font-bold mt-1">3,240</div><div className="text-xs text-emerald-600 mt-1">▲ 8.2%</div></div>
              <div className="kpi"><div className="text-xs text-slate-500">Avg Handle Time</div><div className="text-2xl font-bold mt-1">02:34</div><div className="text-xs text-red-600 mt-1">▼ 3s</div></div>
            </div>
            <div className="px-8 mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4 pb-8">
              <div className="card p-4 lg:col-span-2">
                <h3 className="font-semibold mb-3">Call Volume (Last 7 Days)</h3>
              ...
              <div className="card p-4">
                ...
                <h3 className="font-semibold mt-5 mb-3">Quick Actions</h3>
                <div className="grid grid-cols-2 gap-2">
                  <button className="btn-outline" onClick={() => switchPage('users')}>👥 Users</button>
                  <button className="btn-outline" onClick={() => switchPage('numbers')}>📱 DIDs</button>
                  <button className="btn-outline" onClick={() => switchPage('tenants')}>🏢 Tenants</button>
                  <button className="btn-outline" onClick={() => switchPage('logs')}>📄 Logs</button>
                </div>
              </div>
            </div>
          </section>
            ...
            <div className="px-8 mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4 pb-8">
              <div className="card p-5">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <div className="text-xs text-slate-500">Line</div>
                    <select className="select mt-1"><option>(345) 616-1256</option><option>(408) 637-1715</option></select>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-slate-500">Status</div>
                    <div className={`text-sm font-semibold ${callState.active ? 'text-red-600' : 'text-emerald-600'}`}>● {callState.active ? 'On Call' : 'Ready'}</div>
                  </div>
                </div>
                <input className="input text-2xl text-center font-mono py-4 mb-1" placeholder="Enter number" value={spNumber} onChange={(e) => setSpNumber(e.target.value)} />
                <div className="text-center text-xs text-slate-500 mb-3 h-4">{spTimer}</div>
                <div className="grid grid-cols-3 gap-2 mb-4">
                  {[['1',''],['2','ABC'],['3','DEF'],['4','GHI'],['5','JKL'],['6','MNO'],['7','PQRS'],['8','TUV'],['9','WXYZ'],['*',''],['0','+'],['#','']].map((btn) => (
                    <button key={btn[0]} className="dial-btn" onClick={() => dialPad(btn[0])}>{btn[0]}<span className="sub">{btn[1]}</span></button>
                  ))}
                </div>
                <div className="flex gap-2">
                  <button className={`flex-1 py-3 ${callState.active ? 'btn-danger' : 'btn-primary'}`} onClick={toggleCall}>{callState.active ? '✕ End Call' : '📞 Call'}</button>
                  <button className="btn-outline px-3" onClick={dialBackspace}>⌫</button>
                </div>
                <div className={`grid grid-cols-4 gap-2 mt-4 ${callState.active ? '' : 'opacity-40 pointer-events-none'}`}>
                  <button className={`ctrl-btn ${callState.mute ? 'on' : ''}`} onClick={() => { setCallState({ ...callState, mute: !callState.mute }); toggleCtl('mute'); }}>🔇<span>Mute</span></button>
                  <button className={`ctrl-btn ${callState.hold ? 'on' : ''}`} onClick={() => { setCallState({ ...callState, hold: !callState.hold }); toggleCtl('hold'); }}>⏸<span>Hold</span></button>
                  <button className={`ctrl-btn ${callState.rec ? 'rec' : ''}`} onClick={() => { setCallState({ ...callState, rec: !callState.rec }); toggleCtl('rec'); }}>⏺<span>Record</span></button>
                  <button className={`ctrl-btn ${callState.spk ? 'on' : ''}`} onClick={() => { setCallState({ ...callState, spk: !callState.spk }); toggleCtl('spk'); }}>🔊<span>Speaker</span></button>
              ...
              <div className="card p-5 lg:col-span-2">
                ...
                {!callState.active ? (
                  ...
                  <div>
                    <div className="flex items-center gap-4 pb-4 border-b">
                      <div className="avatar" style={{ width: '56px', height: '56px', fontSize: '1.2rem', background: '#3b82f6' }}>{spNumber.slice(-2)}</div>
                      <div className="flex-1 min-w-0"><div className="font-bold text-lg truncate">Outbound Call</div><div className="text-sm text-slate-500">{spNumber}</div></div>
                    ...
                    <div className="mt-4"><div className="text-xs text-slate-500 mb-1">Live Waveform</div><canvas ref={waveCanvasRef} height={60} className="w-full border rounded" /></div>
                  </div>
                )}
                <h3 className="font-semibold mt-6 mb-2">Recent</h3>
                <div className="space-y-2">
                  {recentCalls.slice(0, 5).map((r, i) => (
                    <div key={i} className="flex items-center justify-between p-2 border rounded">
                      <div><div className="font-medium text-sm">{r.num}</div><div className="text-xs text-slate-500">{r.time} · {r.dur}</div></div>
        ...
        {activePage === 'outbound' && (
          <section className="page active">
            ...
            <div className="px-8 mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="kpi"><div className="text-xs text-slate-500">Calls Today</div><div className="text-2xl font-bold">248</div></div>
            ...
            <div className="px-8 mt-6 flex flex-wrap gap-2">
              <button className="btn-primary" onClick={() => showToast('New campaign created')}>+ New Campaign</button>
              <button className="btn-outline" onClick={() => switchPage('softphone')}>📞 Open Dialer</button>
              <button className="btn-outline" onClick={() => setBulkDialModal(true)}>📋 Bulk Dial</button>
              <button className="btn-outline" onClick={() => showToast('Exported CSV')}>⬇ Export</button>
            </div>
            <div className="px-8 mt-4 pb-8">
              <div className="card table-wrap">
                <table><thead><tr><th>Time</th><th>Agent</th><th>Number</th><th>Duration</th><th>Outcome</th><th>Campaign</th><th>Actions</th></tr></thead>
                <tbody>
                  {obData.map((r, i) => {
                    const chipClass = r[4] === 'Connected' ? 'chip-green' : r[4] === 'Voicemail' ? 'chip-yellow' : 'chip-red';
                    return (
                      <tr key={i}><td>{r[0]}</td><td>{r[1]}</td><td>{r[2]}</td><td>{r[3]}</td>
                        <td><span className={`chip ${chipClass}`}>{r[4]}</span></td><td>{r[5]}</td>
        ...
        {activePage === 'inbound' && (
          <section className="page active">
            ...
            <div className="px-8 mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="kpi"><div className="text-xs text-slate-500">Ringing</div><div className="text-2xl font-bold text-blue-600">{inboundCalls.length}</div></div>
            ...
            <div className="px-8 mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4 pb-8">
              <div className="card p-4 lg:col-span-2">
                <div className="flex items-center justify-between mb-3"><h3 className="font-semibold">Live Incoming</h3><button className="btn-outline" onClick={SimulateIncoming}>+ Simulate Ring</button></div>
                <div className="space-y-2">
                  {inboundCalls.map((c, i) => (
                    <div key={i} className="flex items-center gap-3 p-3 border rounded animate-pulse-slow">
                      <span className="status-dot" style={{ background: '#3b82f6' }} />
                      <div className="flex-1 min-w-0"><div className="font-semibold truncate">{c.name}</div><div className="text-xs text-slate-500">{c.num} · {c.queue} · waiting {c.wait}s</div></div>
                      <button className="btn-primary" onClick={() => { setSpNumber(c.num); setInboundCalls(inboundCalls.filter((_, j) => j !== i)); switchPage('softphone'); setTimeout(() => toggleCall(), 100); }}>Answer</button>
                      <button className="btn-outline" onClick={() => { setInboundCalls(inboundCalls.filter((_, j) => j !== i)); showToast('Call declined'); }}>Decline</button>
                    </div>
                  ))}
                  {inboundCalls.length === 0 && <div className="text-center text-slate-500 py-6">No incoming calls</div>}
                </div>
              ...
              <div className="card p-4">
                ...
                <div className="space-y-2 text-sm">
                  {[
                    { name: 'Auto-answer after 3 rings', def: true }, { name: 'Route to IVR first', def: true },
                    { name: 'Voicemail if no agent', def: true }, { name: 'Business hours only', def: false },
                    { name: 'Record all calls', def: true },
                  ].map((rule, i) => (
        ...
        {activePage === 'queue' && (
          <section className="page active">
            ...
            <div className="px-8 mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="kpi"><div className="text-xs text-slate-500">In Queue</div><div className="text-2xl font-bold">{queueData.length}</div></div>
              <div className="kpi"><div className="text-xs text-slate-500">Longest Wait</div><div className="text-2xl font-bold text-orange-600">{queueData.length ? fmtT(Math.max(...queueData.map((q) => q.wait))) : '00:00'}</div></div>
            ...
            <div className="px-8 mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4 pb-8">
              <div className="card p-4">
                <div className="flex items-center justify-between mb-3"><h3 className="font-semibold">Waiting Callers</h3><button className="btn-outline" onClick={() => showToast('Queue refreshed')}>↻ Refresh</button></div>
                <div className="space-y-2">
                  {queueData.map((q, i) => (
                    <div key={i} className="queue-row">
                      <span className="status-dot" style={{ background: q.prio === 'High' ? '#dc2626' : '#3b82f6' }} />
                      <div className="flex-1 min-w-0"><div className="font-medium truncate">{q.name}</div><div className="text-xs text-slate-500">{q.num} · {q.queue}</div></div>
                      <div className="text-sm font-mono">{fmtT(q.wait)}</div>
                      <button className="btn-primary" onClick={() => { setSpNumber(q.num); setQueueData(queueData.filter((_, j) => j !== i)); switchPage('softphone'); setTimeout(() => toggleCall(), 100); }}>Pick</button>
                    </div>
                  ))}
                  {queueData.length === 0 && <div className="text-center text-slate-500 py-4">Queue empty</div>}
                </div>
                  ...
                  {agents.map((a, i) => {
                    ...
                    return (
                      <div key={i} className="queue-row">
                        ...
                        <div className="flex-1 min-w-0"><div className="font-medium truncate">{a.name}</div><div className="text-xs text-slate-500 capitalize">{a.status}</div></div>
                        <span className="status-dot" style={{ background: col }} />
                      </div>
                    );
                  })}
        ...
        {activePage === 'billing' && hasPermission('canViewBilling') && (
          <section className="page active">
            <div className="px-8 pt-6">
              <h1 className="text-2xl font-bold">Global Billing</h1>
              <p className="text-slate-500 text-sm">Role: {roleLabels[userRole]} | Modify Billing: {hasPermission('canModifyBilling') ? 'Allowed' : 'View-only'}</p>
            </div>
            <div className="px-8 mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            {hasPermission('canModifyBilling') && (
              <div className="px-8 mt-6 flex flex-wrap gap-2">
            ...
            <div className="px-8 mt-4 pb-8">
              <div className="card table-wrap">
                <table><thead><tr><th>Invoice</th><th>Date</th><th>Amount</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                  {billData.map((r, i) => (
                    <tr key={i}>
                      <td className="font-medium">{r.invoice}</td><td>{r.date}</td><td>{r.amount}</td>
                      <td><span className={`chip ${r.status === 'Paid' ? 'chip-green' : 'chip-yellow'}`}>{r.status}</span></td>
                      <td><button className="btn-outline" onClick={() => showToast(`Invoice ${r.invoice} downloaded`)}>⬇ PDF</button></td>
                    </tr>
                  ))}
                </tbody></table>
              </div>
        ...
        {activePage === 'security' && hasPermission('canViewSecurity') && (
          <section className="page active">
            <div className="px-8 pt-6"><h1 className="text-2xl font-bold">Security</h1><p className="text-slate-500 text-sm">Role: {roleLabels[userRole]}</p></div>
            <div className="px-8 mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4 pb-8">
              <div className="card p-4">
                <h3 className="font-semibold mb-3">Security Settings</h3>
                <div className="space-y-3 text-sm">
                ...
                <div className="space-y-2 text-sm">
                  {audit.map((a, i) => (
                    <div key={i} className="flex items-start gap-3 border-b pb-2">
                      ...
                      <div className="flex-1 min-w-0"><div className="text-sm"><b>{a.who}</b> {a.what}</div><div className="text-xs text-slate-500">{a.when} · {a.ip}</div></div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {activePage === 'users' && hasPermission('canManageUsers') && (
          <section className="page active">
            <div className="px-8 pt-6 flex items-start justify-between flex-wrap gap-2">
              <div><h1 className="text-2xl font-bold">User Management</h1><p className="text-slate-500 text-sm">Manage workspace users and permissions • Role: {roleLabels[userRole]}</p></div>
              <button className="btn-blue" onClick={() => { setCreateUserModal(true); setUserForm({ name: '', role: 'Agent', status: 'Active' }); }}>+ Add New User</button>
            </div>
            <div className="px-8 mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="kpi"><div className="text-xs text-slate-500">Total Users</div><div className="text-2xl font-bold">{users.length}</div></div>
            ...
            <div className="px-8 mt-6 pb-8">
              <div className="card mt-4 p-3 flex flex-wrap gap-3 items-center">
                <input className="input flex-1 min-w-[180px] max-w-md" placeholder="Search users..." value={userSearch} onChange={(e) => setUserSearch(e.target.value)} />
                <select className="select max-w-xs" value={userFilterStatus} onChange={(e) => setUserFilterStatus(e.target.value)}><option>All</option><option>Active</option><option>Disabled</option></select>
                <button className="btn-outline" onClick={() => showToast('Users exported')}>⬇ Export</button>
              </div>
              <div className="card mt-3 table-wrap">
                <table><thead><tr><th><input type="checkbox" /></th><th>Username</th><th>Role</th><th>Status</th><th>Owner</th><th>Created</th><th>Actions</th></tr></thead>
                <tbody>
                  {filteredUsers.map((u, i) => (
                    <tr key={i}>
                      <td><input type="checkbox" /></td>
        ...
        {activePage === 'numbers' && hasPermission('canManageDIDs') && (
          <section className="page active">
            ...
            <div className="px-8 mt-6 pb-8">
              ...
              <div className="card mt-3 table-wrap">
                <table><thead><tr><th>DID ID</th><th>Phone Number</th><th>Type</th><th>Status</th><th>Tenant</th><th>Extension</th><th>Provider</th><th>Created</th><th>Actions</th></tr></thead>
                <tbody>
                  {filteredDids.map((did, i) => (
                    <tr key={i}>
                      <td className="font-medium whitespace-nowrap">{did.id}</td>
                      ...
                      <td><span className={`chip ${did.type === 'Inbound' ? 'chip-blue' : 'chip-green'}`}>{did.type}</span></td>
                      <td><span className="chip chip-green">Active</span></td>
                      <td>{did.tenant}</td><td>{did.extension}</td><td>{did.provider}</td><td>{did.created}</td>
                      <td className="whitespace-nowrap">
                        <button className="btn-outline" onClick={() => { setSelectedDid(did); setDidForm({ phone: did.phone, type: did.type, tenant: did.tenant, extension: did.extension, provider: did.provider }); setEditDidModal(true); }}>✎ Edit</button>
                        <button className="btn-outline" onClick={() => assignDidToTenant(did.id, 'TENANT001')}>🔗 Assign</button>
                        <button className="btn-outline" onClick={() => deleteDid(did.id)}>🗑 Delete</button>
        ...
        {activePage === 'tenants' && hasPermission('canManageTenants') && (
          <section className="page active">
            ...
            <div className="px-8 mt-6 pb-8">
              ...
              <div className="card mt-3 table-wrap">
                <table><thead><tr><th>Tenant ID</th><th>Name</th><th>Owner</th><th>Status</th><th>Users</th><th>DIDs</th><th>Tier</th><th>Created</th><th>Actions</th></tr></thead>
                <tbody>
                  {filteredTenants.map((tenant, i) => (
                    <tr key={i}>
                      <td className="font-medium whitespace-nowrap">{tenant.id}</td>
                      <td className="font-semibold">{tenant.name}</td><td>{tenant.owner}</td>
                      <td><span className={`chip ${tenant.status === 'Active' ? 'chip-green' : 'chip-gray'}`}>{tenant.status}</span></td>
                      <td>{tenant.users}</td><td>{tenant.dids}</td>
                      <td><span className="chip chip-blue">{tenant.tier}</span></td><td>{tenant.created}</td>
                      <td className="whitespace-nowrap">
                        <button className="btn-outline" onClick={() => { setSelectedTenant(tenant); setTenantForm({ name: tenant.name, owner: tenant.owner, tier: tenant.tier }); setEditTenantModal(true); }}>✎ Edit</button>
                        <button className="btn-outline" onClick={() => toggleTenantStatus(tenant.id)}>{tenant.status === 'Active' ? 'Disable' : 'Enable'}</button>
                        <button className="btn-outline" onClick={() => deleteTenant(tenant.id)}>🗑 Delete</button>
        ...
        {activePage === 'omni' && (
          <section className="page active">
            ...
            <div className="px-8 mt-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[{ icon: '∞', label: 'Meta', color: '#1877f2' }, { icon: '💬', label: 'WhatsApp', color: '#25d366' }, { icon: '✉', label: 'SMS', color: '#334155' }, { icon: '📧', label: 'Email', color: '#3b82f6' }].map((c, i) => (
                  <button key={i} className="card p-6 flex flex-col items-center gap-2 hover:shadow-md transition" onClick={() => showToast(c.label + ' selected')}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6 mb-8">
                <div className="card p-4"><h3 className="font-semibold mb-3">Combined Omnichannel Usage (Last 30 Days)</h3><canvas ref={omniChartRef} height={300} /></div>
                <div className="card p-4">
                  <h3 className="font-semibold mb-3">WhatsApp Performance</h3>
                  <div className="table-wrap">
                    <table><thead><tr><th>Number</th><th>User</th><th>Sent</th><th>Recv</th><th>Rate</th><th>Avg</th></tr></thead>
                    <tbody>
                      {[['(123) 456…','Amy Smith','190','268','98.7%','17.2s'],['(123) 456…','Amy Smith','336','400','98.7%','17.3s'],['(123) 456…','Mny Number','220','258','98.7%','12.3s']].map((r, i) => (
        ...
        {activePage === 'ivr' && (
          <section className="page active">
            <div className="px-8 pt-6"><h1 className="text-2xl font-bold">IVR Menu Builder — Main Company Hotline</h1></div>
            <div className="px-8 mt-6 pb-8">
              <div className="card p-4">
                <h3 className="font-semibold mb-3">IVR Builder</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-3">
                    <div className="text-xs font-semibold uppercase text-slate-500">Trigger</div>
                    {ivrNodes.trigger.map((n) => (
                      <div key={n.id} className="ivr-node relative group">
                        <button className="absolute top-1 right-1 text-xs opacity-0 group-hover:opacity-100" onClick={() => removeIVRNode(n.id, 'trigger')}>✕</button>
                        <button className="w-full text-left" onClick={() => { setEditingIvrNode(n); setIvrEditModal(true); }}>
                          <div className="node-title">{n.icon} {n.title}</div>
                          <div className="mt-2 text-xs whitespace-pre-line text-slate-600">{n.detail}</div>
                        </button>
                      </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {ivrNodes.flow.map((n) => (
                      <div key={n.id} className="ivr-node relative group">
                        <button className="absolute top-1 right-1 text-xs opacity-0 group-hover:opacity-100" onClick={() => removeIVRNode(n.id, 'flow')}>✕</button>
                        <button className="w-full text-left" onClick={() => { setEditingIvrNode(n); setIvrEditModal(true); }}>
                          <div className="node-title">{n.icon} {n.title}</div>
                          <div className="mt-2 text-xs whitespace-pre-line text-slate-600">{n.detail}</div>
                        </button>
                    ...
                    <button className="btn-outline w-full" onClick={() => addIVRNode('flow')}>+ Add flow step</button>
                  </div>
                  <div className="space-y-3">
                    <div className="text-xs font-semibold uppercase text-slate-500">Keypad Inputs</div>
                    <div className="ivr-node bg-slate-50"><div className="node-title">☰ Keypad Inputs</div><div className="text-xs mt-1 text-slate-500">Press digit → destination</div></div>
                    {ivrNodes.inputs.map((k, i) => (
                      <div key={i} className="flex items-center gap-2">
                        <input className="input" style={{ width: '46px', textAlign: 'center', padding: '.35rem' }} maxLength={1} value={k.key} onChange={(e) => updateKeypadNode(i, e.target.value, k.node.title, k.node.icon)} />
                        <div className="ivr-node flex-1 relative group">
                          <button className="absolute top-1 right-1 text-xs opacity-0 group-hover:opacity-100" onClick={() => removeKeypadInput(i)}>✕</button>
                          <button className="w-full text-left" onClick={() => { setEditingIvrNode(k.node); setIvrEditModal(true); }}>
                            <div className="node-title">{k.node.icon} {k.node.title}</div>
                          </button>
                        </div>
                      </div>
                    ))}
                    <button className="btn-outline w-full" onClick={addKeypadInput}>+ Add keypad input</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        {activePage === 'logs' && (
          <section className="page active">
            <div className="px-8 pt-6"><div className="text-xs text-slate-500">Global Call Logs</div><h1 className="text-2xl font-bold">Master Call History</h1></div>
            <div className="px-8 mt-6 pb-8">
              <div className="card mt-3 table-wrap">
                <table><thead><tr><th><input type="checkbox" /></th><th>Date &amp; Time</th><th>User</th><th>Number</th><th>Dir</th><th>Duration</th><th>Queue</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                  {logsData.map((r, i) => {
                    const a = agents.find((x) => x.name === r[1]) || { color: '#64748b' };
                    return (
                      <tr key={i}>
                        ...
                        <td className="whitespace-nowrap"><span className="avatar mr-2" style={{ background: a.color }}>{initials(r[1])}</span>{r[1]}</td>
                        <td>{r[2]}</td><td>{r[3] === 'in' ? '↙ in' : '↗ out'}</td><td>{r[4]}</td><td>{r[5]}</td>
                        <td>{statusChip(r[6])}</td>
                        <td className="whitespace-nowrap">
      ...
      {testCallModal && (
        <div className="modal-backdrop open" onClick={() => setTestCallModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold">Test Call Control</h3><button onClick={() => setTestCallModal(false)} className="text-slate-500">✕</button></div>
            <div className="space-y-3">
              <div><label className="text-xs font-medium">From (Outgoing)</label><select className="select" ref={tcFromRef}><option>(345) 616-1256</option><option>(41) 616-1256</option><option>(3) 414-3453</option></select></div>
              <div><label className="text-xs font-medium">To (Customer)</label><input className="input" ref={toNumberRef} placeholder="+1 (___) ___-____" /></div>
              <button className="w-full btn-primary py-3" onClick={() => { const toNum = toNumberRef.current?.value || 'unknown'; const fromNum = tcFromRef.current?.value || ''; setSpNumber(toNum); setTestCallModal(false); showToast(`Test call from ${fromNum} → ${toNum}`); switchPage('softphone'); }}>Initiate Test Call</button>
            </div>
          </div>
        </div>
      )}
      ...
      {incomingModal && (
        <div className="modal-backdrop open" onClick={() => setIncomingModal(false)}>
          <div className="modal text-center" onClick={(e) => e.stopPropagation()}>
            <div className="avatar mx-auto mb-3" style={{ width: '70px', height: '70px', fontSize: '1.5rem', background: '#3b82f6' }}>{incomingCallInfo.avatar}</div>
            <div className="text-lg font-bold">{incomingCallInfo.name}</div>
            <div className="text-slate-500 mb-6">{incomingCallInfo.num}</div>
            <div className="flex gap-3 justify-center">
      ...
      {ivrEditModal && editingIvrNode && (
        <div className="modal-backdrop open" onClick={() => setIvrEditModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold">Edit IVR Node</h3><button onClick={() => setIvrEditModal(false)} className="text-slate-500">✕</button></div>
            <div className="space-y-3">
              <div><label className="text-xs font-medium">Icon</label><input className="input" value={editingIvrNode.icon} onChange={(e) => setEditingIvrNode({ ...editingIvrNode, icon: e.target.value })} /></div>
              <div><label className="text-xs font-medium">Title</label><input className="input" value={editingIvrNode.title} onChange={(e) => setEditingIvrNode({ ...editingIvrNode, title: e.target.value })} /></div>
              <div><label className="text-xs font-medium">Description</label><textarea className="input" rows={3} value={editingIvrNode.detail} onChange={(e) => setEditingIvrNode({ ...editingIvrNode, detail: e.target.value })} /></div>
              <button className="w-full btn-primary py-3" onClick={() => { showToast('IVR Node updated'); setIvrEditModal(false); }}>Save Node</button>
            </div>
          </div>
        </div>
      )}
      ...
      {permissionsModal && (
        <div className="modal-backdrop open" onClick={() => setPermissionsModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold">User Permissions: {selectedUser}</h3><button onClick={() => setPermissionsModal(false)} className="text-slate-500">✕</button></div>
            <div className="space-y-3">
              {Object.entries(userPerms).map(([key, value]) => (
                <label key={key} className="flex items-center justify-between">
                  <span>{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                  <label className="toggle"><input type="checkbox" checked={value as boolean} onChange={() => {}} /><span></span></label>
                </label>
              ))}
              <button className="w-full btn-primary py-3" onClick={() => { showToast('Permissions saved for ' + selectedUser); setPermissionsModal(false); }}>Save Permissions</button>
            </div>
          </div>
        </div>
      )}
      ...
      {bulkDialModal && (
        <div className="modal-backdrop open" onClick={() => setBulkDialModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold">Bulk Dial</h3><button onClick={() => setBulkDialModal(false)} className="text-slate-500">✕</button></div>
            <div className="space-y-3">
              <div><label className="text-xs font-medium">Phone Numbers (one per line)</label><textarea className="input" rows={6} value={bulkNumbers} onChange={(e) => setBulkNumbers(e.target.value)} placeholder="(555) 111-0001&#10;(555) 111-0002&#10;(555) 111-0003" /></div>
              <button className="w-full btn-primary py-3" onClick={bulkDial}>Queue Bulk Dial</button>
            </div>
          </div>
        </div>
      )}
      ...
      {createDidModal && (
        <div className="modal-backdrop open" onClick={() => setCreateDidModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-lg">Create New DID</h3><button onClick={() => setCreateDidModal(false)} className="text-slate-500">✕</button></div>
            <div className="space-y-3">
              <div><label className="text-xs font-medium">Phone Number *</label><input className="input" placeholder="e.g., (555) 123-4567" value={didForm.phone} onChange={(e) => setDidForm({...didForm, phone: e.target.value})} /></div>
              <div><label className="text-xs font-medium">Type *</label><select className="select" value={didForm.type} onChange={(e) => setDidForm({...didForm, type: e.target.value as 'Inbound' | 'Outbound'})}><option>Inbound</option><option>Outbound</option></select></div>
              <div><label className="text-xs font-medium">Extension *</label><input className="input" placeholder="e.g., ext-001" value={didForm.extension} onChange={(e) => setDidForm({...didForm, extension: e.target.value})} /></div>
              <div><label className="text-xs font-medium">Tenant</label><select className="select" value={didForm.tenant} onChange={(e) => setDidForm({...didForm, tenant: e.target.value})}>{tenants.map(t => <option key={t.id}>{t.name}</option>)}</select></div>
              <div><label className="text-xs font-medium">Provider</label><select className="select" value={didForm.provider} onChange={(e) => setDidForm({...didForm, provider: e.target.value})}><option>Twilio</option><option>Vonage</option><option>Bandwidth</option></select></div>
              <button className="w-full btn-primary py-3" onClick={createDid}>Create DID</button>
            </div>
          </div>
        </div>
      ...
      {editDidModal && selectedDid && (
        <div className="modal-backdrop open" onClick={() => setEditDidModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-lg">Edit DID {selectedDid.id}</h3><button onClick={() => setEditDidModal(false)} className="text-slate-500">✕</button></div>
            <div className="space-y-3">
              <div><label className="text-xs font-medium">Phone Number</label><input className="input" value={didForm.phone} onChange={(e) => setDidForm({...didForm, phone: e.target.value})} /></div>
              <div><label className="text-xs font-medium">Type</label><select className="select" value={didForm.type} onChange={(e) => setDidForm({...didForm, type: e.target.value as 'Inbound' | 'Outbound'})}><option>Inbound</option><option>Outbound</option></select></div>
              <div><label className="text-xs font-medium">Extension</label><input className="input" value={didForm.extension} onChange={(e) => setDidForm({...didForm, extension: e.target.value})} /></div>
              <div><label className="text-xs font-medium">Tenant</label><select className="select" value={didForm.tenant} onChange={(e) => setDidForm({...didForm, tenant: e.target.value})}>{tenants.map(t => <option key={t.id}>{t.name}</option>)}</select></div>
              <button className="w-full btn-primary py-3" onClick={updateDid}>Update DID</button>
            </div>
          </div>
      ...
      {createTenantModal && (
        <div className="modal-backdrop open" onClick={() => setCreateTenantModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-lg">Create New Tenant</h3><button onClick={() => setCreateTenantModal(false)} className="text-slate-500">✕</button></div>
            <div className="space-y-3">
              <div><label className="text-xs font-medium">Tenant Name *</label><input className="input" placeholder="e.g., Acme Corporation" value={tenantForm.name} onChange={(e) => setTenantForm({...tenantForm, name: e.target.value})} /></div>
              <div><label className="text-xs font-medium">Owner Email *</label><input className="input" type="email" placeholder="owner@example.com" value={tenantForm.owner} onChange={(e) => setTenantForm({...tenantForm, owner: e.target.value})} /></div>
              <div><label className="text-xs font-medium">Subscription Tier</label><select className="select" value={tenantForm.tier} onChange={(e) => setTenantForm({...tenantForm, tier: e.target.value as 'Starter' | 'Professional' | 'Enterprise'})}><option>Starter</option><option>Professional</option><option>Enterprise</option></select></div>
              <button className="w-full btn-primary py-3" onClick={createTenant}>Create Tenant</button>
            </div>
          </div>
        </div>
      ...
      {editTenantModal && selectedTenant && (
        <div className="modal-backdrop open" onClick={() => setEditTenantModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-lg">Edit Tenant {selectedTenant.id}</h3><button onClick={() => setEditTenantModal(false)} className="text-slate-500">✕</button></div>
            <div className="space-y-3">
              <div><label className="text-xs font-medium">Tenant Name</label><input className="input" value={tenantForm.name} onChange={(e) => setTenantForm({...tenantForm, name: e.target.value})} /></div>
              <div><label className="text-xs font-medium">Owner Email</label><input className="input" type="email" value={tenantForm.owner} onChange={(e) => setTenantForm({...tenantForm, owner: e.target.value})} /></div>
              <div><label className="text-xs font-medium">Subscription Tier</label><select className="select" value={tenantForm.tier} onChange={(e) => setTenantForm({...tenantForm, tier: e.target.value as 'Starter' | 'Professional' | 'Enterprise'})}><option>Starter</option><option>Professional</option><option>Enterprise</option></select></div>
              <button className="w-full btn-primary py-3" onClick={updateTenant}>Update Tenant</button>
            </div>
          </div>
      ...
      {createUserModal && (
        <div className="modal-backdrop open" onClick={() => setCreateUserModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-lg">Create New User</h3><button onClick={() => setCreateUserModal(false)} className="text-slate-500">✕</button></div>
            <div className="space-y-3">
              <div><label className="text-xs font-medium">Username *</label><input className="input" placeholder="e.g., John_Doe" value={userForm.name} onChange={(e) => setUserForm({...userForm, name: e.target.value})} /></div>
              <div><label className="text-xs font-medium">Role *</label>
                <select className="select" value={userForm.role} onChange={(e) => setUserForm({...userForm, role: e.target.value})}>
                  <option>Agent</option><option>Supervisor</option><option>Tenant Admin</option><option>Billing</option>
                </select>
              </div>
              <button className="w-full btn-primary py-3" onClick={createUser}>Create User</button>
            </div>
      ...
      {editUserModal && selectedUserData && (
        <div className="modal-backdrop open" onClick={() => setEditUserModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-lg">Edit User {selectedUserData.name}</h3><button onClick={() => setEditUserModal(false)} className="text-slate-500">✕</button></div>
            <div className="space-y-3">
              <div><label className="text-xs font-medium">Username</label><input className="input" value={userForm.name} onChange={(e) => setUserForm({...userForm, name: e.target.value})} /></div>
              <div><label className="text-xs font-medium">Role</label>
                <select className="select" value={userForm.role} onChange={(e) => setUserForm({...userForm, role: e.target.value})}>
                  <option>Agent</option><option>Supervisor</option><option>Tenant Admin</option><option>Billing</option>
                </select>
              </div>
              <div><label className="text-xs font-medium">Status</label><select className="select" value={userForm.status} onChange={(e) => setUserForm({...userForm, status: e.target.value as 'Active' | 'Disabled'})}><option>Active</option><option>Disabled</option></select></div>
              <button className="w-full btn-primary py-3" onClick={updateUser}>Update User</button>
            </div>
          </div>
        </div>
